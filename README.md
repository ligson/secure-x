...
聊天大文件和文件模块分享复用加密文件分片上传链路：客户端按分片加密上传密文文件，聊天消息只发送端到端加密的文件引用；文件模块可把已有密文文件分享给好友或群聊，服务端只追加密文下载授权，不接触文件密钥或文件名明文。详见 [doc/08-encrypted-file-sharing.md](doc/08-encrypted-file-sharing.md)。

语音/视频通话媒体链路采用自托管 `LiveKit`：secure-x 后端从 YAML 配置读取 LiveKit 地址与 API 密钥，在校验好友关系或共同群成员关系后签发短期房间 token；客户端通过实时配置接口获取 RTC 服务信息并加入 LiveKit 房间。部署上可以选择 LiveKit 内置 TURN/TLS 与公网 `443/TCP` 复用的最简方案，减少移动网络和防火墙环境下的连通性问题。若同一个公网 `443/TCP` 还承载普通 HTTPS 站点，入口层必须按独立 TURN 域名做四层 SNI 分流，不能用 HTTP `location` 规则处理 TURN/TLS；否则客户端可能把 HTTPS 反代入口当作 TURN/TLS 端点，导致音视频媒体 ICE 失败。
