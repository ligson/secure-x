...
- `securex-app/`
  - Flutter 多端客户端
  - 已实现后端地址配置、注册、登录、登录密码修改、解锁密码解锁与修改、密码分类管理、文件目录、登录项管理、聊天会话本机加密队列、好友通讯录与申请处理、随机密码生成、加密文件后台分片上传与进度展示、重命名、删除与解密下载，以及面向移动网络的实时通道自动恢复
...
聊天能力按“好友在线才实时发送、服务端不落消息”的方向设计：客户端通过 HTTP 配置接口获取实时服务信息，优先使用 WebRTC DataChannel 做点对点实时通道；在移动网络、切网、弱网或 NAT 导致 P2P 不稳定时，会自动回退到“客户端先加密、服务端仅在线转发”的 WebSocket 中继模式。无论是直连还是中继，聊天正文都会使用客户端协商出的会话密钥再做 AES-GCM 加密；好友不在线时，消息只加密保存在发送方本机。

登录解锁后，客户端会自动建立实时连接，并在应用回到前台、手机 Wi‑Fi/蜂窝网络切换、弱网恢复后尝试自动恢复在线状态、好友会话和历史同步。历史消息仍只从在线好友或共同群成员的本机拉取，服务端不保存聊天正文或历史消息。
